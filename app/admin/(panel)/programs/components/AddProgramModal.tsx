...
export default function AddProgramModal({ open, onClose, onSuccess }: Props) {
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center z-50 ${open ? "bg-black/40 pointer-events-auto" : "pointer-events-none opacity-0"}`}>
      <div className="bg-white p-6 rounded w-full max-w-md">
        <h2 className="text-lg font-semibold mb-4">Tambah Program</h2>
        ...
        <div className="mb-6">
          ...
          <input key={judul} type="file" accept="image/*" className="w-full mt-1" onChange={(e) => setFile(e.target.files ? e.target.files[0] : null)} />
        </div>
