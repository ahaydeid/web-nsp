...
export default function TeachersSection() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data } = await supabase.from("teachers").select("id, name, title, expertise, experience, photo").order("id", { ascending: true });
  ...
  const loopItems = teachers.concat(teachers);
    ...
    <div className="relative w-full py-24 px-4 mx-auto max-w-6xl text-slate-800 overflow-hidden">
      ...
      <div className="relative w-full overflow-hidden mt-16">
        <div className="flex gap-12 animate-slide">
          {loopItems.map((t, i) => (
            <div key={`${t.id}-${i}`} className="shrink-0 w-[300px] sm:w-[320px] lg:w-[340px]">
              <div className="relative w-full flex flex-col items-center text-center px-6 pt-10 pb-12">
                ...
                <div className="relative w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-md">
                  <Image src={t.photo || "/img/profil-placeholder.png"} alt={t.name} fill className="object-cover" />
                </div>
                ...
                <p className="mt-1 text-blue-800 text-sm">{t.experience}</p>
              </div>
            </div>
          ))}
        </div>
